...
import { Send, ArrowLeft, Home, BrainCircuit, Database, type LucideIcon } from 'lucide-react';
import Link from 'next/link';

const ICONS: Record<string, LucideIcon> = {
  management: BrainCircuit,
  sql: Database,
};
...
function displayLabel(topic: string): string {
  ...
  if (sep !== -1) return topic.slice(sep + 3).replace(/^### /, '');
  return topic;
}
...
export default function QuizPage({ subject, title }: QuizPageProps) {
  const Icon = ICONS[subject] ?? Database;
  ...
  const startQuiz = useCallback(() => {
    ...
    const filtered = selectedTopic ? quizBank.filter(q => q.topic === selectedTopic) : quizBank;
  ...
  const sendAnswer = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/${subject}/quiz-evaluate`, {
        method: 'POST',
  ...
  }, [input, loading, currentQuestion, messages, activeBank, askedIds, subject]);
  ...
  const headerSub = `${activeTopicLabel} · ${questionCount}Q · ${totalCents.toFixed(2)}¢`;
